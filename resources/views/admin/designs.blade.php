        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                <div class="flex items-center">
                    <h1 class="text-xl font-semibold text-gray-900 dark:text-white">Designs</h1>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8 border-b border-gray-200 dark:border-zinc-700">
            <nav class="flex space-x-8" aria-label="Tabs">
                ...
                <a href="{{ route('admin.designs') }}" class="border-blue-500 text-blue-500 px-1 py-4 text-sm font-medium border-b-2">
                    Designs
                </a>
        ...
        <div class="mb-8">
            <a href="{{ route('admin.designs.create') }}" class="bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-600 text-white px-6 py-3 rounded-xl font-medium transition-all duration-200 hover:shadow-lg hover:shadow-blue-500/20 flex items-center gap-2">
        ...
        <!-- Designs Table List View -->
        <div class="bg-white dark:bg-zinc-800 overflow-hidden rounded-xl shadow">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-zinc-700">
                <thead>
                    <tr>
                        <th scope="col" class="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Design</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-zinc-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-zinc-700/50 transition-colors duration-200">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="h-10 w-10 flex-shrink-0">
                                    <img class="h-10 w-10 rounded-lg object-cover" src="{{ $design->image_path ?: 'https://placehold.co/100x100' }}" alt="">
                                </div>
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $design->title }}</div>
                                    <div class="text-sm text-gray-500">{{ Str::limit($design->description, 50) }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ $design->created_at ? $design->created_at->format('M d, Y') : '' }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-3">
                                <a href="{{ route('admin.designs.edit', $design->id) }}" class="text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-300">Edit</a>
                                <button type="button" class="text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-300 delete-btn" data-action="{{ route('admin.designs.destroy', $design->id) }}">Delete</button>
                            </div>
                        </td>
                    </tr>
                    @endforeach
        ...
        <div id="deleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 hidden">
            <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-xl p-8 w-full max-w-md">
                ...
                <p class="text-gray-600 dark:text-gray-300 mb-6">Are you sure you want to delete this design? This action cannot be undone.</p>
